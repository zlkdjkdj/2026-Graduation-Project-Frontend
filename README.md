...
**`dist` 폴더**는 **Distribution(배포)**의 약자로, 프로젝트를 빌드한 후 **실제 웹 서버에 올려 사용자들에게 서비스할 때 사용하는 최종 결과물 파일들이 모이는 폴더**입니다.

* **최종 빌드 결과물 저장**: `pnpm run build` 실행 시 TypeScript(`.tsx`), Tailwind CSS 등을 브라우저가 직접 해석할 수 있는 일반 HTML, CSS, JS 파일로 컴파일하여 `dist` 폴더에 생성합니다.
* **성능 및 용량 최적화**: 공백, 주석 등을 제거하는 압축(Minify) 과정과 파일들을 하나로 묶는 번들링(Bundling)을 진행합니다. 브라우저 캐싱 문제를 방지하기 위해 파일명 뒤에 고유한 해시값(예: `index-CgmH3h1O.js`)이 추가됩니다.
* **실제 배포 대상**: Vercel, Netlify, AWS S3 등의 호스팅 서버에 배포할 때는 프로젝트 전체가 아닌 이 `dist` 폴더 내의 완성된 정적 파일들만 배포합니다.
* **버전 관리 제외**: 빌드 명령어 실행 시마다 동적으로 생성 및 갱신되므로 `.gitignore`에 등록하여 Git 레포지토리 관리 대상에서 제외합니다.
