...
const MOCK_RANKING: RankUser[] = [{ rank: 1, name: '김코딩', points: 1540 }, { rank: 2, name: '박공부', points: 1420 }, { rank: 3, name: '이헬스', points: 1280 }];
const MOCK_ROUTINES: RoutineSuccess[] = [{ name: '공부 진도 준수', rate: 82, color: 'bg-indigo-500' }, { name: '주 4회 이상 운동', rate: 65, color: 'bg-orange-500' }];
    ...
    <div className="space-y-8 animate-slideUp">
      <div className={`p-8 md:p-12 rounded-[3rem] border ${t.card} bg-gradient-to-br from-indigo-600/10 to-purple-600/10`}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="space-y-4">
            ...
            <div className="flex gap-3"><div className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-black">LV. {Math.floor(userPoints / 100)}</div></div>
          </div>
        ...
        <div className={`p-8 rounded-[3rem] border ${t.card} lg:col-span-2`}>
          ...
          {MOCK_ROUTINES.map(r => (
            <div key={r.name} className="mb-4">
              <div className="flex justify-between text-sm font-semibold mb-1"><span>{r.name}</span><span>{r.rate}%</span></div>
              <div className="h-4 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full ${r.color}`} style={{ width: `${r.rate}%` }} /></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`p-7 rounded-[2.5rem] border ${t.card} flex gap-4`}>
        ...
        <div className="flex-1 space-y-4">
          <textarea value={newPostContent} onChange={e => setNewPostContent(e.target.value)} className="w-full bg-transparent border-none text-sm font-semibold focus:ring-0 outline-none" rows={2} placeholder="오늘의 열정을 한 줄로 남겨보세요!" />
          <button onClick={handleAddPost} className="px-7 py-2.5 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase ml-auto block">Share Progress</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {posts.map(p => (
          <div key={p.id} className={`p-7 rounded-[2.5rem] border ${t.card}`}>
             <div className="flex justify-between mb-4"><span className="font-bold">{p.author}</span><span className="text-xs font-black">LV. {Math.floor(p.points / 100)}</span></div>
             <p className="text-sm font-medium mb-5">{p.content}</p>
             <div className="flex gap-5 border-t pt-4"><button className="flex items-center gap-1.5 text-rose-400"><Heart size={15} /> {p.likes}</button></div>
          </div>
        ))}
      </div>
    </div>
