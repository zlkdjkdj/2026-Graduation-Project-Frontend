...
export function MainPage() {
  ...
  const modes = [
    ...
    { id: 'community' as Mode, name: 'Community', description: '자랑하기', color: 'emerald' }, // 4개 꽉 채우기 위해 추가
  ];
...
// --- 2. Fitness Routine Content (건드리지 않음) ---
...
// --- 3. Miracle Time Content (건드리지 않음) ---
...
function CommunityContent() {
  const [posts, setPosts] = useState<Post[]>([
    ...
    { id: 3, userName: '공부왕라이언', userRank: 3, content: '알고리즘 3문제 풀이 성공. 내일은 더 일찍 일어나야지.', completedRoutines: ['알고리즘', '영어공부'], successRate: 90, likes: 15, reactions: {'👏': 5}, category: '#공부', streakDays: 20, createdAt: '5시간 전' },
  ]);
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <section className="bg-white rounded-3xl border-2 border-emerald-50 p-6 shadow-sm">
        <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
            <span className="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600">✍️</span>
            오늘의 갓생 자랑하기
        </h4>
        <textarea 
          className="w-full h-28 p-4 bg-gray-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-sm resize-none"
          placeholder="오늘 어떤 루틴을 성공하셨나요? 소감을 남겨보세요!"
        ...
        <div className="mt-4 flex flex-wrap gap-2 items-center justify-between">
            <div className="flex gap-2">
                {["#미라클모닝", "#운동", "#공부"].map(cat => (
                    <button 
                        ...
                        className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all ${selectedCategory === cat ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                    >
                        {cat}
                    </button>
                ))}
            ...
            <button 
                ...
                className="px-6 py-2.5 bg-gray-900 text-white font-bold rounded-xl text-xs hover:bg-black active:scale-95 transition-all shadow-lg"
            >
                인증글 올리기
            </button>
        </div>
